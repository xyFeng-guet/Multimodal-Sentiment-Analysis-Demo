...
class CrossModuleTransformer(nn.Module):
    """
    ...
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0, res_dropout=0.0,
        ...
        for layer in range(layers):
            l2other_new = TransformerEncoderLayer(
                embed_dim,
                num_heads=num_heads,
            ...
            other2l_new = TransformerEncoderLayer(
                embed_dim,
                num_heads=num_heads,
                attn_dropout=attn_dropout,
                relu_dropout=relu_dropout,
    ...
    def forward(self, seq_l, seq_other, lengths=None, mask=None):
        """Forward 2 input modals thorugh the Trnasformer
        Args:
        ...
        origin_l, origin_other = seq_l, seq_other
        cross_layer = math.ceil(self.layers / 3)
        # add residual connection
        # resl_all = []
        # resother_all = []

        for layer_i, (trans_l2other, trans_other2l) in enumerate(zip(self.l2other_layers, self.other2l_layers)):
            # project other modals to language
            other2l = trans_l2other(input_l, x_k=origin_other, x_v=origin_other, lengths=lengths, mode='o2l')

            # project language to other modals
            l2other = trans_other2l(input_other, x_k=origin_l, x_v=origin_l, lengths=lengths, mode='l2o')
            ...
            if layer_i >= cross_layer:
                origin_l, origin_other = other2l, l2other

        return other2l, l2other  # placeholder for fusion output
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None, lengths=None, mode='l2o'):
        """
        Args:
...
if __name__ == '__main__':
    encoder = CrossModuleTransformer(300, 4, 2)
